...
int rc_hash_xbox(char hash[33], const rc_hash_iterator_t* iterator){
  md5_state_t md5;
  void* file_handle;

  const uint32_t SECTOR_SIZE = 2048;
  const uint32_t STARTING_SECTOR = 264;

  /* Buffer to hold individual sector */
  ...
  rc_file_seek(iterator, file_handle, SECTOR_SIZE * STARTING_SECTOR, SEEK_SET);
  rc_file_read(iterator, file_handle, sector_buff, SECTOR_SIZE);

  /* Check magic bytes */
  if(memcmp(sector_buff, "MICROSOFT*XBOX*MEDIA", 20) != 0){
    rc_file_close(iterator, file_handle);
    return rc_hash_iterator_error(iterator, "Unable to find volume descriptor.");
  }

  /* Get the sector containing the root directory table */
  uint32_t directory_table_sector;
  memcpy(&directory_table_sector, &sector_buff[20], 4);

  /* Get the root directory table */
  rc_file_seek(iterator, file_handle, SECTOR_SIZE * directory_table_sector, SEEK_SET);
  rc_file_read(iterator, file_handle, sector_buff, SECTOR_SIZE);

  /* There should be a file in the root directory. Get the length of that file's name. */
  uint8_t file_name_len;
  memcpy(&file_name_len, &sector_buff[0xd], 1);

  /* Extract file name */
  uint8_t file_name[file_name_len];
    memcpy(&file_name, &sector_buff[0xe], file_name_len);

  /* This file should be called default.xbe. Check to make sure it is. */
  if(memcmp(file_name, "default.xbe", 11) != 0){
    rc_file_close(iterator, file_handle);
    return rc_hash_iterator_error(iterator, "No default.xbe in root folder.");
  }

  /* Get the sector containing default.xbe*/
  uint32_t default_xbe_sector;
  memcpy(&default_xbe_sector, &sector_buff[0x4], 4);

  /* Get the file size of default.xbe */
  uint32_t default_xbe_file_size_in_bytes;
  memcpy(&default_xbe_file_size_in_bytes, &sector_buff[0x8], 4);

  /* Get the number of sectors default.xbe takes up. */
  uint32_t default_xbe_file_size_in_sectors;
  if(default_xbe_file_size_in_bytes % SECTOR_SIZE != 0){
      default_xbe_file_size_in_sectors = (default_xbe_file_size_in_bytes / SECTOR_SIZE) + 1;
  }
  else{
      default_xbe_file_size_in_sectors = default_xbe_file_size_in_bytes / SECTOR_SIZE;
  }

  /* Create buffer to store default.xbe */
  uint8_t* default_xbe_buffer;
  default_xbe_buffer = (uint8_t*)malloc(default_xbe_file_size_in_sectors);
  if (!default_xbe_buffer) {
    rc_file_close(iterator, file_handle);
    return rc_hash_iterator_error(iterator, "Could not allocate temporary buffer");
  }

  /* Read default.xbe into buffer */
  rc_file_seek(iterator, file_handle, SECTOR_SIZE * default_xbe_file_size_in_sectors, SEEK_CUR);
  rc_file_read(iterator, file_handle, default_xbe_buffer, default_xbe_file_size_in_bytes);

  /* Check magic bytes */
  if(memcmp(default_xbe_buffer, "XBEH", 4) != 0){
    rc_file_close(iterator, file_handle);
    return rc_hash_iterator_error(iterator, "File default.xbe is not a valid .xbe file.");
  }

  /* Finally hash the darn thing */
  rc_hash_iterator_verbose_formatted(iterator, "Hashing %d bytes of default.xbe.", default_xbe_file_size_in_bytes);
  md5_append(&md5, default_xbe_buffer, default_xbe_file_size_in_bytes);

  /* Close or free resources */
  rc_file_close(iterator, file_handle);
  free(default_xbe_buffer);
