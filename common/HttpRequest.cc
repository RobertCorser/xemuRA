...
static size_t header_callback(char* buffer, size_t size, size_t nitems, void* userdata)
{
  ...
  const std::string_view value = [full_buffer, colon_pos] -> std::string_view{
    std::string_view str = full_buffer.substr(colon_pos + 1);
                                               ...
                                               std::span<Multiform> multiform)
{
  ...
  if (response_code != 200)
  {
    if (buffer.empty())
    {
      //TODO: Get the request type, url, and response code and print them.
      fprintf(stderr, "Error communicating with server. TODO: GET THE RESPONSE CODE!!!");
      //ERROR_LOG_FMT(COMMON, "Failed to {} {}: server replied with code {}", type, url,
      //              response_code);
    }
    else
    {
      //TODO: Get the error code, error message, and type of request that failed (i.e. GET or POST)
      fprintf(stderr, "Failed to GET or POST. Server replied with error message. TODO: GET THE ERROR MESSAGE!!!");
