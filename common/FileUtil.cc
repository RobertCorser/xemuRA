...
#include "common/CommonFuncs.hh"
#include "common/CommonPaths.hh"
#include "common/CommonTypes.hh"
#ifdef __APPLE__
#include "Common/DynamicLibrary.h"
#endif
#include "common/IOFile.hh"
...
{
  //TODO: Change these debug statements (present in most of these functions) to print when log level is set to debug
  ...
  if (!fs::exists(status))
  {
    ...
    {
      fprintf(stderr, "Delete: %s does not exist\n",  filename.c_str());
    }
    return true;
  }
  ...
  {
    fprintf(stderr, "Delete failed: %s is a directory\n", filename.c_str());
    return false;
  }
...
{
  //fprintf(stdout, "CreateDir: directory %s\n", path.c_str());
  ...
  if (!success && fs::is_directory(native_path, error_ignored))
    success = true;
  if (!success)
    fprintf(stderr, "CreateDir: failed on %s: %s\n", path.c_str(), error.message().c_str());
  return success;
}

bool CreateDirs(std::string_view path)
{
  //fprintf(stdout, "CreateDirs: directory %s\n", std::string(path).c_str());
  ...
  if (!success && fs::is_directory(native_path, error_ignored))
    success = true;
  if (!success)
    fprintf(stderr, "CreateDirs: failed on %s: %s\n", std::string(path).c_str(), error.message().c_str());
  return success;
}

bool CreateFullPath(std::string_view fullPath)
{
  //fprintf(stdout, "CreateFullPath: path %s\n", std::string(fullPath).c_str());
  ...
  if (!success && fs::is_directory(native_path, error_ignored))
    success = true;
  if (!success)
    fprintf(stderr, "CreateFullPath: failed on %s: %s\n", std::string(fullPath).c_str(), error.message().c_str());
  return success;
}
...
{
  //fprintf(stdout, "DeleteDir: directory %s\n", filename.c_str());
  ...
  if (!fs::exists(status))
  {
    if (behavior == IfAbsentBehavior::ConsoleWarning)
    {
      fprintf(stdout, "DeleteDir: %s does not exist\n", filename.c_str());
    }
    return true;
  }
  ...
  if (!fs::is_directory(status))
  {
    fprintf(stderr, "DeleteDir: Not a directory %s\n", filename.c_str());
    return false;
  }

  if (!fs::remove(native_path, error))
  {
    fprintf(stdout, "DeleteDir: failed on %s: %s\n", filename.c_str(), error.message().c_str());
    return false;
  }
...
{
  //fprintf(stdout, "%s: %s --> %s", srcFilename, destFilename);
  std::error_code error;
...
bool RenameSync(const std::string& srcFilename, const std::string& destFilename)
{
  ...
  {
    if (_commit(fd) != 0)
      //TODO: Convert errno_t err to an int or string and print it
      //fprintf(stderr, "%s sync failed on %s: %s", srcFilename, err);
      fprintf(stderr, "RenameSync sync failed on %s: NEED TO IMPLEMENT PRINTING THIS ERROR\n", srcFilename.c_str());
    close(fd);
  }
#else
...
{
  //fprintf(stdout, "%s: %s --> %s", source_path, destination_path);
  ...
  if (!copied)
  {
    fprintf(stderr, "CopyRegularFile: failed %s --> %s: %s\n", std::string(source_path).c_str(),
...
u64 GetSize(FILE* f)
{
  ...
  {
    //fprintf(stderr, "GetSize: seek failed %s: %s", fmt::ptr(f), Common::LastStrerrorString());
    fprintf(stderr, "GetSize: seek failed %p: %s\n", fmt::ptr(f), Common::LastStrerrorString().c_str());
    return 0;
  }
...
{
  //fprintf(stdout, "CreateEmptyFile: %s", filename);

  if (!File::IOFile(filename, "wb"))
  {
    fprintf(stderr, "CreateEmptyFile: failed %s: %s\n", filename.c_str(), Common::LastStrerrorString().c_str());
    return false;
  }
...
{
  //fprintf(stdout, "%s: directory %s", directory);
  ...
  if (error)
  {
    ...
    fprintf(stderr, "ScanDirectoryTree error on %s: %s\n", directory.c_str(), error.message().c_str());
  }
...
bool DeleteDirRecursively(const std::string& directory)
{
  ...
  if (!success)
    fprintf(stderr, "DeleteDirRecursively: %s failed. %s\n", directory.c_str(), error.message().c_str());
  return success;
}

bool Copy(std::string_view source_path, std::string_view dest_path, bool overwrite_existing)
{
  //fprintf(stdout, "%s: %s --> %s (%s)", source_path, dest_path,
  ...
  if (error)
  {
    ...
    fprintf(stderr, "Copy: failed %s --> %s (%s): %s\n", std::string(source_path).c_str(), 
                                                       std::string(dest_path).c_str(),
                                                       overwrite_existing ? "overwrite" : "preserve", 
                                                       error.message().c_str());
    return false;
  }
...
{
  //fprintf(stdout, "%s: %s --> %s", source_path, dest_path);
  auto src_path = StringToPath(source_path);
  auto dst_path = StringToPath(dest_path);
  std::error_code error;
  if (!MoveWithOverwrite(src_path, dst_path, error))
  {
    fprintf(stderr, "MoveWithOverwrite: failed %s --> %s: %s\n", std::string(source_path).c_str(), 
                                                               std::string(dest_path).c_str(),
                                                               error.message().c_str());
  }
  return true;
...
std::string GetCurrentDir()
{
  ...
  if (error)
  {
    fprintf(stderr, "GetCurrentDir failed: %s\n", error.message().c_str());
    return {};
  }
  return directory;
}
...
bool SetCurrentDir(const std::string& directory)
{
  ...
  if (error)
  {
    fprintf(stderr, "SetCurrentDir failed: %s\n", error.message().c_str());
    return false;
  }
  return true;
}
...
#define SYSDATA_DIR "sys"
  ...
  fprintf(stdout, "CreateSysDirectoryPath: Setting to %s\n", sys_directory.c_str());
...
{
  INFO_LOG_FMT(COMMON, "Setting Sys directory to %s", path);
  ASSERT_MSG(COMMON, s_android_sys_directory.empty(), "Sys directory already set to %s",
...
{
  INFO_LOG_FMT(COMMON, "Setting Driver directory to %s and library path to %s", path, lib_path);
  ASSERT_MSG(COMMON, s_android_driver_directory.empty(), "Driver directory already set to %s",
             s_android_driver_directory);
  ASSERT_MSG(COMMON, s_android_lib_directory.empty(), "Library directory already set to %s",
