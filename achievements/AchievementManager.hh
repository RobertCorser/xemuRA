...
#include "common/HookableEvent.hh"
// #include "Common/Lazy.h"
#include "common/WorkQueueThread.hh"
...
public:
    using BadgeNameFunction =
        std::function<std::string(const AchievementManager &)>;
    ...
    static constexpr std::string_view DEFAULT_PLAYER_BADGE_FILENAME =
        "achievements_player.png";
    static constexpr std::string_view DEFAULT_GAME_BADGE_FILENAME =
        "achievements_game.png";
    static constexpr std::string_view DEFAULT_LOCKED_BADGE_FILENAME =
    ...
    std::string_view game_id, u16 revision) const; void
    FilterApprovedGeckoCodes(std::vector<Gecko::GeckoCode>& codes,
    std::string_view game_id, u16 revision) const; void
    FilterApprovedARCodes(std::vector<ActionReplay::ARCode>& codes,
    std::string_view game_id, u16 revision) const; bool
    ShouldGeckoCodeBeActivated(const Gecko::GeckoCode& code, std::string_view
    game_id, u16 revision) const; bool ShouldARCodeBeActivated(const
    ActionReplay::ARCode& code, std::string_view game_id, u16 revision) const;
    bool IsApprovedGeckoCode(const Gecko::GeckoCode& code, std::string_view
    game_id, u16 revision) const; bool IsApprovedARCode(const
    ActionReplay::ARCode& code, std::string_view game_id, u16 revision) const;
    ...
    */
    ...
    {
        return rc_client_raintegration_has_modifications(m_client);
    }
#endif // RC_CLIENT_SUPPORTS_RAINTEGRATION
...
private:
    ...
    static size_t FilereaderRead(void *file_handle, void *buffer,
    ...
    static void LoginCallback(int result, const char *error_message,
                              rc_client_t *client, void *userdata);

    void FetchBoardInfo(AchievementId leaderboard_id);

    //std::unique_ptr<DiscIO::Volume> &GetLoadingVolume()
    //{
    //    return m_loading_volume;
    //}

    static void LoadGameCallback(int result, const char *error_message,
                                 rc_client_t *client, void *userdata);
    static void ChangeMediaCallback(int result, const char *error_message,
                                    rc_client_t *client, void *userdata);
    ...
    template <typename T>
    void FilterApprovedIni(std::vector<T> &codes, std::string_view game_id,
                           u16 revision) const;
    template <typename T>
    bool ShouldCodeBeActivated(const T &code, std::string_view game_id,
                               u16 revision) const;
    template <typename T>
    bool IsApprovedCode(const T &code, std::string_view game_id,
                        u16 revision) const;
    ...
    static void
    LeaderboardEntriesCallback(int result, const char *error_message,
                               rc_client_leaderboard_entry_list_t *list,
                               rc_client_t *client, void *userdata);

    static void
    HandleAchievementTriggeredEvent(const rc_client_event_t *client_event);
    static void
    HandleLeaderboardStartedEvent(const rc_client_event_t *client_event);
    static void
    HandleLeaderboardFailedEvent(const rc_client_event_t *client_event);
    static void
    HandleLeaderboardSubmittedEvent(const rc_client_event_t *client_event);
    static void
    HandleLeaderboardTrackerUpdateEvent(const rc_client_event_t *client_event);
    static void
    HandleLeaderboardTrackerShowEvent(const rc_client_event_t *client_event);
    static void
    HandleLeaderboardTrackerHideEvent(const rc_client_event_t *client_event);
    static void HandleAchievementChallengeIndicatorShowEvent(
        const rc_client_event_t *client_event);
    static void HandleAchievementChallengeIndicatorHideEvent(
        const rc_client_event_t *client_event);
    static void HandleAchievementProgressIndicatorShowEvent(
        const rc_client_event_t *client_event);
    static void HandleGameCompletedEvent(const rc_client_event_t *client_event,
                                         rc_client_t *client);
    ...
    static void Request(const rc_api_request_t *request,
                        rc_client_server_callback_t callback,
                        void *callback_data, rc_client_t *client);
    static u32 MemoryPeeker(u32 address, u8 *buffer, u32 num_bytes,
                            rc_client_t *client);
    //void FetchBadge(Badge *badge, u32 badge_type, BadgeNameFunction function,
    //                UpdatedItems callback_data);
    static void EventHandler(const rc_client_event_t *event,
                             rc_client_t *client);

#ifdef RC_CLIENT_SUPPORTS_RAINTEGRATION
    static void LoadIntegrationCallback(int result, const char *error_message,
                                        rc_client_t *client, void *userdata);
    static void
    RAIntegrationEventHandler(const rc_client_raintegration_event_t *event,
                              rc_client_t *client);
    static void MemoryPoker(u32 address, u8 *buffer, u32 num_bytes,
                            rc_client_t *client);
    ...
    std::chrono::steady_clock::time_point m_last_rp_time =
        std::chrono::steady_clock::now();
    std::chrono::steady_clock::time_point m_last_progress_message =
        std::chrono::steady_clock::now();
...
public:
    ...
    constexpr bool IsHardcoreModeActive()
    {
        return false;
    }
