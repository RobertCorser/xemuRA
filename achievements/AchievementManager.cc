...
static const Common::HttpRequest::Headers USER_AGENT_HEADER = {
    //TODO: Get version from xemu.c
    {"User-Agent", "xemu/0.8.134"}};
...
void AchievementManager::Init(void* hwnd)
{
  ...
  if (!m_client && g_config.rcheevos.enable_ra)
  {
    ...
    //SetHardcoreMode();
    //TODO: Use xemu setting to enable/disable retroachievements
...
void AchievementManager::Login(const std::string& password)
{
  ...
  if (password.empty())
  {
    rc_client_begin_login_with_token(m_client, g_config.rcheevos.username,
                                     g_config.rcheevos.api_token, LoginCallback,
                                     nullptr);
  }
  else
  {
    rc_client_begin_login_with_password(m_client, g_config.rcheevos.username,
                                       ...
                                       void* userdata)
{
  ...
  if (result != RC_OK)
  {
    fprintf(stderr, "Failed to login %s to RetroAchievements server.\n",
                 g_config.rcheevos.username);
    g_config.rcheevos.api_token = "";
    return;
  }
  ...
  if (!user)
  {
    fprintf(stderr, "Failed to retrieve user information from client.\n");
    return;
  }
  ...
  fprintf(stdout, "Successfully logged in %s to RetroAchievements server.\n",
               user->username);
  std::lock_guard lg{instance.GetLock()};
  g_config.rcheevos.api_token = user->token;
